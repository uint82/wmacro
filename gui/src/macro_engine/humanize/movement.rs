//! classifies movements by distance and selects velocity profiles per class.
...
pub fn choose_velocity_profile(class: MovementClass, rng: &mut Pcg) -> VelocityProfile {
    ...
    match class {
        ...
        MovementClass::Medium => {
            if r < 0.55 {
                VelocityProfile::MinJerk
            } else if r < 0.80 {
                VelocityProfile::LogNormal
            } else if r < 0.95 {
    ...
    match profile {
        ...
        VelocityProfile::Corrective => {
            ...
            if u < hesitate_at {
                ...
                hesitate_at * 0.9
                    + min_jerk((u - hesitate_at) / (1.0 - hesitate_at)) * (1.0 - hesitate_at * 0.9)
            }
        }
    }
}
...
pub fn generate_waypoints(
    ...
    if dist > 0.0 && n_waypoints > 0 {
        ...
        for i in 1..=n_waypoints {
            ...
            let perp_d = rng
                .next_normal(0.0, dist * 0.04)
                .clamp(-dist * 0.10, dist * 0.10);
