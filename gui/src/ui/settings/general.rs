...
pub fn render(ui: &mut egui::Ui, state: &SharedState, palette: &ThemePalette) {
    ui.label(egui::RichText::new("Application-wide settings and appearance.").color(palette.text_muted).size(11.0));
    ui.add_space(10.0);

    egui::Grid::new("general_grid").num_columns(2).spacing([16.0, 12.0]).show(ui, |ui| {
        ui.label(egui::RichText::new("Theme").color(palette.text_primary).size(13.0));

        let (current_theme, available_themes) = {
            let Ok(state_guard) = state.lock() else { return };
            (state_guard.theme_name.clone(), state_guard.theme_manager.available_themes.clone())
        };
            ...
            egui::ComboBox::from_id_salt("theme_selector")
                ...
                .show_ui(ui, |ui| {
                    for theme in available_themes {
                        ui.selectable_value(&mut selected_theme, theme.name.clone(), &theme.name)
